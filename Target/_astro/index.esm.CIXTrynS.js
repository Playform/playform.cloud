...
const Ce = (t, e) => e.some((n) => t instanceof n);
let P, F;
function ve() {
	return (
		P ||
...
function Me(t) {
	return t === IDBDatabase.prototype.transaction &&
		...
		? function (e, ...n) {
				...
				return X.set(s, e.sort ? e.sort() : [e]), f(s);
			}
		: Ae().includes(t)
			? function (...e) {
					return t.apply(w(this), e), f(Y.get(this));
				}
			: function (...e) {
	...
	return (
		s &&
			i.addEventListener("upgradeneeded", (c) => {
				s(f(i.result), c.oldVersion, c.newVersion, f(i.transaction), c);
			}),
		n &&
...
const d = new Se("@firebase/app"),
	...
	Ge = "@firebase/data-connect",
	Je = "@firebase/database-compat",
	Ke = "@firebase/functions",
	Ye = "@firebase/functions-compat",
	Xe = "@firebase/installations",
	Ze = "@firebase/installations-compat",
	qe = "@firebase/messaging",
	Qe = "@firebase/messaging-compat",
	et = "@firebase/performance",
	tt = "@firebase/performance-compat",
	nt = "@firebase/remote-config",
	rt = "@firebase/remote-config-compat",
	st = "@firebase/storage",
	at = "@firebase/storage-compat",
	it = "@firebase/firestore",
	ot = "@firebase/vertexai",
...
function $(t) {
	...
	if (M.has(e))
		return (
			d.debug(`There were multiple attempts to register component ${e}.`),
			!1
		);
	M.set(e, t);
...
function vt(t, e = {}) {
	...
	const s = Object.assign(
			{ name: lt, automaticDataCollectionEnabled: !1 },
			e,
		),
		r = s.name;
	if (typeof r != "string" || !r)
		throw p.create("bad-app-name", { appName: String(r) });
	if ((n || (n = he()), !n)) throw p.create("no-options");
	const a = T.get(r);
	if (a) {
		if (v(n, a.options) && v(s, a.config)) return a;
		throw p.create("duplicate-app", { appName: r });
	}
	const i = new Ee(r);
	for (const c of M.values()) i.addComponent(c);
	const o = new pt(n, s, i);
	return T.set(r, o), o;
}
function y(t, e, n) {
	var s;
	let r = (s = dt[t]) !== null && s !== void 0 ? s : t;
	n && (r += `-${n}`);
	const a = r.match(/\s|\//),
		i = e.match(/\s|\//);
	if (a || i) {
 ...
 * limitations under the License.
 */
const mt = "firebase-heartbeat-database",
	gt = 1,
	...
	return (
		C ||
			(C = Ne(mt, gt, {
				...
				throw p.create("idb-open", { originalErrorMessage: t.message });
			})),
		C
	);
}
async function bt(t) {
	try {
		...
		if (e instanceof b) d.warn(e.message);
		else {
			const n = p.create("idb-get", { originalErrorMessage: e?.message });
			d.warn(n.message);
		}
	}
}
async function k(t, e) {
	try {
		...
		if (n instanceof b) d.warn(n.message);
		else {
			const s = p.create("idb-set", { originalErrorMessage: n?.message });
			d.warn(s.message);
		}
	}
}
	...
	async getHeartbeatsHeader() {
		...
		try {
			if (
				...
				return "";
			const n = z(),
				{ heartbeatsToSend: s, unsentEntries: r } = Dt(
					this._heartbeatsCache.heartbeats,
				),
...
function wt(t) {
	$(new D("platform-logger", (e) => new He(e), "PRIVATE")),
		$(new D("heartbeat", (e) => new yt(e), "PRIVATE")),
		y(O, V, t),
		y(O, V, "esm2017"),
		y("fire-js", "");
}
...
//# sourceMappingURL=index.esm.CIXTrynS.js.map
